...
Consult this note before publishing the guide. Canonical application and cache requirements remain in `docs/architecture.md`; this file records operator-specific behavior that is easy to miss.
...
2. Run `minisite deploy --dry-run --profile personal dubai.anthonydisanti.com ./dist`. Review every planned deletion.
3. Run the same command without `--dry-run` while keeping `dist/` unchanged.
4. Wait for the returned invalidation ID against distribution `EU943ZSJ1FOAO`.
5. Compare the live root document byte-for-byte with `dist/index.html`, inspect MIME/cache headers, and smoke-test one chapter and one activity fragment.
6. If verification fails, fix the source, rerun the gates, and deploy again. This toy site deliberately favors a short repair loop over rollback infrastructure.
...
- If an upload exits with `ECONNRESET` and explicitly reports no deletion or invalidation, some objects may already be overwritten. Retry the same unchanged, validated `dist/` once; a successful retry completes the full upload and invalidation. This recovered the favicon release on 6 September 2026.
...
- Minisite deletes old fingerprinted bundles before the invalidation has propagated. That trade-off is accepted for this private vacation toy; repair and redeploy instead of maintaining rollback snapshots.
