...
- What works: `npm run check` passes 156 tests, strict TypeScript, typed ESLint, the production build, content integrity, and static-output inspection. Photo audits pass all 431 manifest rows and 401 JPEGs with zero similarity candidates.
- What works: Browser QA covers the established 390, 999, 1000, and 1440 px interaction gate. The current production pass confirms the three-day arrival label, absent Pause/Resume control, assistive stop instruction, current- and different-slide pagination stops beyond a full cycle, passive hero sheet opening, direct Ras Al Khor sheet loading, zero broken images, and zero horizontal overflow.
- What works: Production is live at `https://dubai.anthonydisanti.com/` through stack `minisite-dubai-anthonydisanti-com-36364597` and CloudFront distribution `EU943ZSJ1FOAO`. The 2026-08-13 release uploaded 430 objects, removed only the two superseded bundles, completed invalidation `IAJXSIZ8K37Q9IM5J6KG58C9TY`, and serves a byte-identical `index.html` with the expected JavaScript/CSS MIME types. A fresh 430-object pre-release rollback snapshot remains at `/tmp/dubai-activities-production-backup.OzrKOZ/` for the current machine/session.
