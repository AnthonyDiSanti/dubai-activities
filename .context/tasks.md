...
- id: 2026-09-13-office-happy-hours — title: Add CityMax and Mama Shelter as liked happy hours — owner: Codex — status: done — last update: 2026-09-13
  ...
  - Release: Published 530 objects, deleted only `assets/index-DjF018sF.js`, and completed invalidation `I9659YMSPG1TS3CI0EHBVDS7P4`. All 13 checked live files match the build with expected MIME/cache headers. Both live sheets show the correct notes; sampled gallery frames load and CityMax dismisses to `#loud`.
  - Naming follow-up: Naming follow-up deployed: use Citymax in cards, reviews and credits, with Brew House at Citymax Business Bay retained in location details. All 267 app tests, four Python tests and full release audits pass. Published 530 objects, deleted only `assets/index-Bt7AyV0P.js`, and completed invalidation `I3EESJSAK65GRYCJJ71A6T5ZZZ`. Live HTML, bundles and both credit catalogs match the build with expected headers; the live sheet shows Citymax and the full location.
  - Follow-up: Full uncommitted diff prepared for commit; no staging or commit. No release work remains. Prior café work is committed in `9a1b451` and already deployed.

- id: 2026-09-13-creek-cafes — title: Combine the Creek Harbour animal cafés — owner: Codex — status: done — last update: 2026-09-13
  - Result: One Tried & liked Vibrissae/Fluffin outing with distinct firsthand verdicts, marina/food pairing, both venues’ practical details and eight preserved, credited photos. Retain `vibrissae`; retire the duplicate `fluffin` activity. Meowtropolis remains Tried.
  - Verification: All 266 app tests and four Python tests, lint, TypeScript, production build, content/static/photo/attribution audits pass. All eight image files match their originals byte-for-byte; browser QA confirms all eight frames load, mobile gallery wrap and dismissal to `#animals`, a single paired entry among seven liked archive cards, no overflow at 390/1440 px, and no console errors.
  - Release: Published 522 objects, deleted only `assets/index-Rqbbw0y_.js` and the four superseded `fluffin-*.jpg` paths, and completed invalidation `IEQ046QCUZRTBQODKW4VDFTMC9`. All 13 checked live files (HTML, CSS/JS, both credit catalogs and all eight gallery photos) match `dist/` with expected MIME/cache headers. The live sheet preserves both verdicts and eight photo controls; sampled cat/dog/café frames load, dismissal returns to `#animals`, exactly one combined card remains, and no console errors occur. Full dirty-tree commit prep is complete; index untouched and no commit performed.
