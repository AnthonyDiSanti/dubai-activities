...
The archive itself follows the same global-sheet history contract as credits: ordinary footer activation pushes a fragment, Back/Forward tracks it, direct-link dismissal removes only the fragment, and close restores the footer trigger. Outcome selection replaces that archive history entry instead of pushing another one, so one close still returns to the underlying guide while Forward restores the exact selected outcome. Opening an activity from an isolated outcome and pressing Back returns to that same isolated archive route. Inactive IDs remain valid detail routes but are excluded from favorites, active chapter filtering, and hero selection.
...
5. Exercise chapter selection, fold/open all, Escape menu dismissal, and both planning filters. From a single expanded chapter, click Open everything and confirm the URL becomes `#everything` without losing the path/query; paste that URL into a fresh tab and confirm all chapters start open. Confirm Back restores the chapter fragment and Forward restores `#everything` with every chapter open. Turn each filter on and off and confirm every visible chapter reopens without adding navigation history. Confirm Plan ahead shows exactly 42 dated or explicit-`ahead` activities, remains mutually exclusive with Tried & liked, and preserves chronological dated ordering. Joke Hub, Comedy at the Speakeasy, The Laughter Factory, Courtyard Playhouse, Karak After Dark, and No Grip must be included because their dates, guest-list rules, or operator warnings create real planning friction; BLU and Amazónico must stay out: BLU is a normal walk-in, while Amazónico's retained booking route is viable same-day and does not carry `ahead`. The verified mode should show exactly the seven `verified` records from `src/data/archive.ts` across four chapters; each thumbs-up stamp stays inside its image without overlapping the preceding card, and every sheet carries the matching text callout.
...
13. Open Tried & decided from the footer. Confirm all three sections begin open and each full-width header folds and reopens its cards. Activate each summary tile and confirm its URL, active treatment, scroll alignment, single expanded group, and one-step dismissal to the guide; use Back/Forward and an archived activity sheet to confirm the isolated group route is restored. Confirm all seven verified records appear only under Tried & liked with the same image-contained thumbs-up stamp as the guide; Meowtropolis, Roberto's, Salmon Guru, and Lock, Stock & Barrel Business Bay appear only under Tried; and The Wall, Mountain Extreme, Brunch & Cake, Butterfly Garden, Fashion Avenue, and The Pods appear only under Rejected. Verify all 17 cards reuse the main treatment styles, show activity-focused previews, omit favorite controls, and open full decision sheets with galleries. Confirm the Soho note says only SOHO Garden was open while HIVE and CODE remain untested, The Pods says it was rejected before a visit, and the Lock, Stock & Barrel sheet and gallery stay specific to Business Bay. Confirm both responsive close controls work and no inactive outcome appears in the guide.
14. Open Photo credits from the footer. Confirm the full 475-asset catalog is readable, external creator/source/license links are present, the loading/error states do not affect the guide, the sticky X and mobile bottom Close work, and focus returns to the footer link.
