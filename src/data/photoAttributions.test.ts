...
describe('generated photo attribution catalog', () => {
  it('covers every deployed asset with the expected attribution mix', () => {
    ...
    expect(catalog.assets).toHaveLength(486);
    expect(filenames.size).toBe(486);
    ...
    expect(basisCounts.source_credit).toBe(403);
    ...
    expect(catalog.assets.find(({ filename }) => filename === 'kartdrome-01.jpg')?.source.name).toBe(
      'Dubai Autodrome',
    );
    expect(catalog.assets.find(({ filename }) => filename === 'nogrip-01.jpg')?.source.name).toBe(
      'No Grip DXB via Fever',
    );
    expect(catalog.assets.find(({ filename }) => filename === 'jokehub-01.jpg')?.source.name).toBe(
      'Mad Cat Comedy',
    );
    expect(catalog.assets.find(({ filename }) => filename === 'speakeasy-01.jpg')?.source.name).toBe(
      'SANA',
    );
    ...
    expect(catalog.assets.find(({ filename }) => filename === 'lolataberna-01.jpg')?.source.name).toBe(
      'Lola Taberna Española',
    );
    expect(catalog.assets.find(({ filename }) => filename === 'salero-01.jpg')?.source.name).toBe(
      'Kempinski Hotel Mall of the Emirates',
    );

    expect(catalog.assets.find(({ filename }) => filename === 'mimikakushi-01.jpg')?.source.name).toBe(
      'Mimi Kakushi',
    );
